...
class TypeChecker(NodeVisitor):
        ...
        try:
            if type1 == 'id':
                ...
                while p is not None:
                    var = p.get(id)
                    if var:
                        if var.__class__.__name__ == "FuncSymbol":
    ...
    def visit_Assignment(self, node):
        item_type = "error" #if we find sth, this will be replaced by a proper type
        ...
        if not item:
            ...
            if item.__class__.__name__ == "FuncSymbol":
            ...
            if res[0] == "id":
                ...
                if var:
                    if var.__class__.__name__ == "FuncSymbol":
                        ...
                        res_type = var.type
            operation = returnType.ttype["="][item_type][res_type]
            if type(operation) == tuple:
                print operation[1],"line:",node.id.lineno, "column:",node.id.column
        except KeyError:
            print "Variable",node.id.id,"has type:", item_type,"but got:",res_type,\
    ...
    def check_if_not_fun_name(self, name):
        p = self.stack
        while p is not None:
            symbol = p.get(name)
            if symbol is not None:
                if symbol.__class__.__name__ == "FuncSymbol":
                    return False
            p = p.parent
        return True
        ...
        if not self.stack.get(node.id.id):
            ...
            for var_desc in fun_vars:
                ...
                if not self.stack.get(var_desc[0]): #if the variable name was not used in the current context
                    if self.check_if_not_fun_name(var_desc[0]):
                        self.stack.put(var_desc[0],var)
                    else:
                        print "Using function",var_desc[0],"as variable, line:",node.id.lineno,"column:",node.id.column
                else:
            ...
            if not self.return_occured:
                print "Missing a proper return statement for function", node.id.id,"present at line:",node.id.lineno,"column:",node.id.column
            self.stack = self.stack.popScope()
        else:
    ...
    def visit_ReturnInstr(self,node):
        ...
        if not scope_desc:
            ...
            if var_ret_type == "id":
                ...
                if var_obj:
                    if var_obj.__class__.__name__ == "FuncSymbol":
                        var_ret_type = "error"
                        print "Assignment to a function? Really? line:", node.id.lineno,"column:",node.id.column
                    else:
    ...
    def visit_DeclaredFunc(self,node):
        ...
        if fundefvar and fundefvar.__class__.__name__ == "FuncSymbol":
    ...
    def visit_Init(self,node):
        if self.check_if_not_fun_name(node.id.id):
            res = (node.id.id,)
        else:
            print "Using function", node.id.id,"as variable, line:",node.id.lineno,"column:",node.id.column
            res = ('error',)
        if node.expr:
            res += self.visit(node.expr)[0]
            #print "init",res
    ...
    def visit_ID(self,node):
        if self.check_if_not_fun_name(node.id):
            return [('id',node.lineno,node.column,node.id)]
        else :
            print "Using function", node.id,"as variable, line:",node.lineno,"column:",node.column
            return [('error', node.lineno,node.column)]
